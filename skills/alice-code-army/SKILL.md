...
- 如果确实需要调整，就应当实际调整，不能只看不动、只写建议不执行。只有在缺少前提、权限、资源，或存在明显风险时，才可以停留在观察和汇报；此时必须明确写出 blocker、风险边界和 `needs_human`。

## 工时统计约束

- 绑定 GitLab `issue` 的 campaign，默认也要维护 issue 工时统计。范围清晰后，应补一个合理的 `estimate`；每轮有实际推进动作时，应同步记录本轮 `spent time`。
- 优先通过 GitLab quick actions 做工时统计：`/estimate` 用于设置或改写总预估，`/spend` 用于追加已花费工时；必要时可修正错误记录，但不要让 estimate 长期失真。
