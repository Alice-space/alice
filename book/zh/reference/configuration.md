...
| 类型 | `string` |
| 默认值 | `"at"` |
| 可选值 | `"at"`、`"prefix"`、`"without_prefix"`、`"all"` |

旧版触发模式。仅在 `group_scenes.chat.enabled` 和 `group_scenes.work.enabled` 均为 `false` 时作为消息闸门使用。

| 值 | 行为 |
|-------|----------|
| `"at"` | 仅接受 @bot 消息 |
| `"prefix"` | 仅接受以 `trigger_prefix` 开头的消息 |
| `"without_prefix"` | 默认接受所有消息，但忽略以 `trigger_prefix` 开头的消息（前缀作为「不要回复」标记） |
| `"all"` | 接受所有消息 |

启用 group_scenes 时 `"without_prefix"` 仍然生效：群消息正文（剥离 bot @ 之后）若以 `trigger_prefix` 开头，会在 scene 路由前被直接丢弃，前缀因此成为全局「不要回这条」的 opt-out 标记。
...
`trigger_mode: "prefix"` 的前缀字符串（必填）；`trigger_mode: "without_prefix"` 的「跳过」前缀字符串（必填）。
