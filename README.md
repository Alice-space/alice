...
- Dashboard includes a **Todo Dialog** panel for interactive todo creation.
- Endpoint `POST /api/v1/todos/dialog` accepts natural-language `message`.
- Todo title is auto-generated by Codex summarization (with fallback summarizer if Codex fails).

## Prompt configuration

All configurable prompts are centralized in:

- `app/prompts.py` (default prompt keys)
- `${ALICE_STATE_DIR}/prompts.json` (runtime override file)

You can override prompt file location with `ALICE_PROMPTS_FILE`.

Inspect active prompts at runtime:

- `GET /api/v1/runtime/prompts`
