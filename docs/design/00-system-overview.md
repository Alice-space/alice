...
5. 自然语言意图模型
   所有人类操作都先由模型解释成结构化 `IntentSpec`，再进入任务流、设置流、审批流或配置提案流；不得用关键词匹配、前缀匹配或正则规则直接决定自然语言意图类别。
...
- 任意人类输入都必须先由模型解释为 `IntentSpec`，不得直接驱动副作用。
- 自然语言意图识别不得以关键词匹配、前缀匹配或正则规则作为主路径或兜底路径。
