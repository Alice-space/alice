...
本文说明如何在本仓库中使用 `skills/` 目录下的能力包，包含编译运行方式，并给出 4 个可直接复用的示例。
...
如果没有现成配置文件，至少保证这些环境变量可用：

```bash
export ALICE_HTTP_LISTEN_ADDR=:8080
export ALICE_STORAGE_ROOT_DIR=./data
export ALICE_WORKFLOW_MANIFEST_ROOTS=configs/workflows
export ALICE_AUTH_ADMIN_TOKEN=dev-admin-token
```
...
然后用 CLI message 入口提交一个简单请求：

```bash
./bin/alice submit message \
...
```bash
./bin/alice list requests --server http://127.0.0.1:8080
./bin/alice get request --server http://127.0.0.1:8080 --request-id <request_id>
```

## 3. 示例 1：天气/公开信息查询（直答路径）
