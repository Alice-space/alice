...
1. 如果 workflow manifest 规定 `plan` 后需要 gate，例如 approval 或 plan review，则 BUS 创建 `ApprovalRequest`。
2. 审批需要几席 reviewer、截止时间多久，都来自 workflow manifest 或策略配置。
3. BUS 只负责创建 gate、跟踪状态、收集结果和记录审计。
4. BUS 不负责在主程序里写死“所有代码任务都必须 plan audit”。
5. 若 gate 通过，task 进入下一个 step；若不通过，则按 manifest 回退到 `plan`，或者进入 `WaitingHuman`。
...
2. BUS 按固定优先级命中当前 `task_id`，优先使用 `repo_ref + issue_ref/pr_ref`，其次才是 `reply_to_event_id` 或对话上下文。
...
1. 人类消息进入入口层。
2. 入口层生成 `ExternalEvent`，记录原始文本、发送人、来源渠道、时间、`conversation_id`、`thread_id`，并附上 `control_object_ref=schedule_registry`；若是修改既有定时任务，还应带 `scheduled_task_id`。
