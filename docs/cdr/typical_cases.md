...
2. 由于命中了 promote 硬规则，这类请求不能停留在 `EphemeralRequest`；BUS 只有在 workflow 候选唯一且约束校验通过时，才会用一个原子动作把 request 标成 `Promoted`，并同时创建 `DurableTask` 和 active `WorkflowBinding`。
3. task 初始信息通常包括：
   - 来源是代码仓库相关请求
   - 关联仓库和 issue/PR
   - 用户请求摘要
