...
要注意，这一节说的是 connector 对“重启意图消息”和“进程恢复”的内部处理；真正的部署侧自更新命令仍然是仓库脚本 `scripts/update-self-and-sync-skill.sh`，它负责 `git pull`、`go build`、`systemctl --user restart` 和同步快照落盘。
