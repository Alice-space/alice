...
`cmd/connector/main.go` 是入口。启动时大致会做这些事：

1. 读取配置文件。
2. 初始化 LLM provider。
3. 同步 bundled skills。
4. 按需注册 MCP server。
5. 调用 `bootstrap.BuildConnectorRuntime` 组装运行时。
6. 同时启动：
...
1. 规范化 `WorkflowPhase`
2. 解析并补充用户名字
3. 处理 restart notification / post-restart finalize 这类特殊 phase
4. 检查是否是内建命令
5. 记录 session scope 和 last message time
6. 决定走“reply 流程”还是“direct send 流程”
...
## 17. 第十四层：重启、自更新、恢复场景的特殊分支

除了普通对话链路，代码里还内置了一套 restart/self-update 相关的处理。

### 17.1 restart intent 识别
...
系统会把它视作 restart intent。

### 17.2 运行过程中正好被重启时

如果当前任务因为应用 shutdown 被 cancel，且这条消息又是 restart intent，那么处理状态不会简单当成普通失败，而是走：

- `post_restart_finalize`
...
这样用户不会看到“刚让它重启，它就突然没下文了”。

## 18. 一个完整示例：群里先发图片，再 @bot 问问题
...
2. `internal/bootstrap/connector_runtime_builder.go`
3. `internal/connector/app.go`
4. `internal/connector/message.go`
5. `internal/connector/media_window.go`
6. `internal/connector/app_queue.go`
7. `internal/connector/processor.go`
8. `internal/connector/processor_context.go`
9. `internal/connector/reply_dispatcher.go`
10. `internal/connector/sender.go`
11. `internal/llm/codex/codex.go`
12. `internal/runtimeapi/server.go`
13. `internal/memory/memory.go`
14. `internal/connector/session_state.go`
15. `internal/connector/app_state.go`
